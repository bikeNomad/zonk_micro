...
class TestEvents < MiniTest::Unit::TestCase
  ...
  def test_event_pattern_matching
    ...
    e = Event.new(1, 2)
    assert_operator(p, :===, e, "simple match must succeed")

    p = EventPattern.new(1, 3)
    e = Event.new(1, 2)
    refute_operator(p, :===, e, "simple mismatch must fail")

    p = EventPattern.new(1, [1, 2])
    e = Event.new(1, 1)
    assert_operator(p, :===, e, "includes? match must succeed")
    e = Event.new(1, 2)
    assert_operator(p, :===, e, "includes? match must succeed")

    p = EventPattern.new(Fixnum, 2)
    e = Event.new(1234, 2)
    assert_operator(p, :===, e, "class/object match must succeed")
    e = Event.new(1234.5, 2)
    refute_operator(p, :===, e, "class/object mismatch must fail")
